...
datadir = 'C:\\Users\\asd47\\OneDrive - rui0828\\master\\CV\\Final_Project\\Code\\paper_2\\ERRNet-master\\'

# datadir_syn = join(datadir, 'VOCdevkit/VOC2012/PNGImages')
datadir_syn = join(datadir, 'training_data\\VOC_224')
# datadir_real = join(datadir, 'real_train')
datadir_real = join(datadir, 'training_data\\Berkeley real dataset\\real\\')
...
eval_dataset_ceilnet = datasets.CEILTestDataset(join(datadir, 'test_data\\testdata_reflection_synthetic_table2'))
eval_dataset_real = datasets.CEILTestDataset(
    join(datadir, 'test_data\\Berkeley real dataset'),
    # fns=read_fns('real_test.txt'))
    fns=os.listdir(join(datadir, 'test_data\\Berkeley real dataset\\blended')))
