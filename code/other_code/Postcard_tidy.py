...
# 定義路徑
base_path = r"C:\Users\asd47\OneDrive - rui0828\master\CV\Final_Project\Code\paper_2\ERRNet-master\test_data\SIR2\Postcard Dataset\Focus"
    ...
    if os.path.isdir(first_folder_path):
        ...
        for second_folder in os.listdir(first_folder_path):
            ...
            if os.path.isdir(second_folder_path):
                # 找到PNG檔案並處理
                for file in os.listdir(second_folder_path):
                    if file.endswith(".png"):
                        if "-g-" in file:
                            shutil.copy(os.path.join(second_folder_path, file), os.path.join(output_folder_g, f"fc_{first_folder}_{second_folder}.png"))
                            # print(file,"g")
                        elif "-m-" in file:
                            shutil.copy(os.path.join(second_folder_path, file), os.path.join(output_folder_m, f"fc_{first_folder}_{second_folder}.png"))
                            # print(file,"m")
                        elif "-r-" in file:
                            shutil.copy(os.path.join(second_folder_path, file), os.path.join(output_folder_r, f"fc_{first_folder}_{second_folder}.png"))
                            # print(file,"r")
